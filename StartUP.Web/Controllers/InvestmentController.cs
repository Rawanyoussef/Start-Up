...
namespace StartUP.Web.Controllers
{
    ...
    public class InvestmentController : ControllerBase
    {
        ...
        public async Task<IActionResult> AcceptInvestment(AcceptDto acceptInvestmentDto)
        {
            var id = Convert.ToInt32(User.FindFirst(ClaimTypes.NameIdentifier)?.Value);
            var user = await _service.AcceptInvestment(acceptInvestmentDto, id);
            return Ok("Investment Accepted successfully.");
        }

        [HttpPost("RejectInvestment")]
        public async Task<IActionResult> DeleteInvestment(AcceptDto acceptInvestmentDto)
        {
            var id = Convert.ToInt32(User.FindFirst(ClaimTypes.NameIdentifier)?.Value);
            var user = await _service.RejectInvestment(acceptInvestmentDto ,id);
