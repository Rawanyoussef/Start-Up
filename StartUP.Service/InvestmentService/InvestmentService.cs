...
namespace StartUP.Service.InvestmentService
{
    public class InvestmentService : IInvestmentService
    {
        ...
        public async Task<int> AddAsync(InvestmentRequestDto investmentdto, int userId)
        {
            ...
            var fullMessage =
               ...
               $"- 📊 Expected Revenue: {investmentdto.Revenue:C}{Environment.NewLine}" +
               $"- 🧮 Equity Percentage: {investmentdto.EquityPercentage}%{Environment.NewLine}" +
               $"- 💵 Interest Rate: {investmentdto.InterestRate}%{Environment.NewLine}" +
               $"- 🤝 Revenue Share: {investmentdto.RevenueShare}%{Environment.NewLine}" +
               $"🎯 Keep pushing forward — your project's growth is in motion!";
        ...
        public async Task<bool> AcceptInvestment(AcceptDto acceptInvestmentDto, int SenderId)
        {
        ...
        public async Task<bool> RejectInvestment(AcceptDto acceptInvestmentDto, int SenderId)
        {
            var investment = await _investRepo.GetByIdAsync(acceptInvestmentDto.Id);
            if (investment == null) throw new Exception("Investment Not Found");
