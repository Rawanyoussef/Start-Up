...
namespace StartUP.Service.ProjectService
{
    public class ProjectService : IProjectService
    {
        ...
        public ProjectService(IProjecRepo projecRepo, IMapper mapper, IHttpContextAccessor httpContextAccessor, IInvestmentRepo investRepo)
        {
        ...
        {
            return await _projecRepo.DeleteAsync(id);
        }
        ...
        public async Task<IEnumerable<ProjectRandomDto>> GetRandomProjectAsync(int count)
        {
            ...
            return projects.Select(p => new ProjectRandomDto
            {
                ...
                Category = p.Category.ToString(),
                Image = string.IsNullOrEmpty(p.Photo)
                    ? null
                    : $"{baseUrl}/images/{p.Photo}",
            });
        }
        ...
        public async Task<IEnumerable<ProjectDtoResponse>> GetByUserNameAsync(string username)
        {
            ...
            foreach (var p in projects)
            {
                ...
                result.Add(new ProjectDtoResponse
                {
                    ...
                    Funding_Source = p.Funding_Source,
                    Budget = p.Budget,
                    Photo = string.IsNullOrEmpty(p.Photo) ? null : $"{baseUrl}/images/{p.Photo}",
                    CreatedAt = p.CreatedAt,
        ...
        public async Task<IEnumerable<ProjectCategoryDto>> GetAllProjectsAsync()
        {
            ...
            foreach (var p in projects)
            {
                ...
                var daysLeft = (p.IsActiveTill - DateTime.UtcNow).Days;
                int numberOfInvestors = await _projecRepo.GetNumberOfInvestorsAsync(p.Id);
                ...
                result.Add(new ProjectCategoryDto
                {
                    ...
                    RaisedOfWhat = $"{totalInvested} of {p.FundingDetails?.TotalInvestment ?? 0}",
                    DaysLeft = daysLeft,
                    CampaignDealType = p.CompanyDeal.DealType,
                    MinInvestment = minInvestment,
                    MaxInvestment = maxInvestment,
                    Goal = p.FundingDetails?.TotalInvestment ?? 0,
                    Category = p.Category.ToString(),
        ...
        public async Task<ProjectCategoryDto> GetProjectByIdAsync(int id)
        {
            ...
            int daysLeft = Math.Max(0, (project.IsActiveTill - DateTime.UtcNow).Days);
         ...
         //   string? latestStatus = await _projecRepo.GetLatestInvestmentStatusAsync(project.Id);
            ...
            return new ProjectCategoryDto
            {
                ...
                Id = project.Id,
                Photo = string.IsNullOrEmpty(project.Photo) ? null : $"{baseUrl}/images/{project.Photo}",
